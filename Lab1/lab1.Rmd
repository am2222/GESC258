...
author: "ASSIGNED in Lab1, January 14th. Exercise Worth: 5% of final grade  "
date: "DUE Date: before Lab 2 on January 21th"
output:
...
**Credit: Part I is mainly based on https://bookdown.org/ndphillips/YaRrr/ **
...
> "Make sure to setup one drive on your remote desktop machine to avoid losing your data. Ask your lab instructor if you need help setting one drive up."
...
```{r cities}
cities <- read.csv("data/canadian_population_centers.csv") #loading dataset into R as a dataframe
```
...
cbind(c('1sd','2sd','3sd'), c("Don't Apply", round(sd2), round(sd3,1)),c(overall_csi.sd1, overall_csi.sd2, overall_csi.sd3) )
...
# add legend
legend(x = min(x.vector2plot)*1.1,
       y = max(h$density)*0.9, 
...
spdf <- geojson_read("data/georef-canada-province.geojson",  what = "sp")
...
#geom_label(data= prov,aes(x=lng, y=lat, label=province_name),
   #       color = "black",label.padding = unit(1, "mm"), check_overlap = T) +
...
https://dataverse.scholarsportal.info/dataset.xhtml?persistentId=doi:10.5683/SP2/OGRTT2 
